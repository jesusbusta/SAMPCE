...
// consulta a la tabla informe_personal	
	$consulta="SELECT * FROM empleado where match (cedula,nombre,apellido,cargo) against ('$criterio*' IN BOOLEAN MODE)";
...
<?PHP
		...
		while ($row = mysql_fetch_row($resultado))
    {
       $link2 =$row[9];
		...
		echo "<tr ><td> C.I: $row[0]</td>";
		echo "<td><a target=_blank href='$link2'><img src='$row[9]' width='120'height='100'></a></td></tr>";
       	echo "<tr><td>Nombre:    $row[1]</td>";
		echo "<td>Apellido:      $row[2]</td></tr>";
		echo "<tr ><td> Cargo: $row[3]</td>";
		echo "<td>Telefono: $row[4]</td><tr >";
       	echo "<tr><td>Direccion: $row[5]</td>";
       	echo "<td>Sexo: $row[6]</td><tr>";
		echo "<tr><td> Horario: $row[8]</td>";
		echo "<td> Fecha: $row[7]</td></tr>";
